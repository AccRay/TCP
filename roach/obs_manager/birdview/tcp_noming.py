...
from roach.utils.misc import get_helper_landmarks, \
    ...
    calculate_speed_magnitue, calculate_acceleration_magnitue
...
class ObsManager():
    ...
    def get_surroundings(self):
        ...
        left_vehicles, mid_vehicles, right_vehicles, back_vehicles = self._distinguish_front_actor_direction(vehicles)
        left_walkers, mid_walkers, right_walkers, _ = self._distinguish_front_actor_direction(walkers)
    ...
    def _distinguish_front_actor_direction(self, actor_list):
        ...
        for actor in actor_list:
            ...
            if angle_to_ego > 175 or angle_to_ego < -175:
                distance = calculate_distance_between_location(ev_loc, loc)
                speed = calculate_acceleration_magnitue(actor)
                forward_vector_to_ego = calculate_direction_angle_between_front_and_current(ev_rot, rot)

                merged_dict = dict(zip(ACTOR_INFO, [distance, angle_to_ego, speed, forward_vector_to_ego]))
                back_actors.append(merged_dict)
        ...
        return left_actors, mid_actors, right_actors, back_actors
