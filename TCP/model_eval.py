...
class TCP(nn.Module):
	...
	def forward(self, is_junction, vehicles, walkers, stops, max_speed, stop_sign, yield_sign,
		...
		att1, feature_emb = self.tf_layer1(cat_emb_feature)
		att2, feature_emb = self.tf_layer2(feature_emb)
		att3, feature_emb = self.tf_layer3(feature_emb)
		att4, feature_emb = self.tf_layer4(feature_emb)
		feature_emb = feature_emb.reshape(-1, 576)
		feature_emb_traj = self.env_feature(F.relu(feature_emb))

		outputs = {}
		outputs['pred_speed'] = self.speed_branch(feature_emb_traj)
		measurement_feature = self.measurements(state)
		
		j_traj = self.join_traj(torch.cat([feature_emb_traj, measurement_feature], 1))
		...
		return outputs, att1, att2, att3, att4
